...
router.post("/", async (req, res) => {
  try {
    ...
    const { category_id, date, title, value } = req.body;
    ...
    if (!date || date.length != 10) {
      return res.status(400).json({
        error: "Date is mandatory and should be in YYYY-MM-DD format.",
      });
    }
    ...
    if (!createResponse.rows[0]) {
      return res.status(400).json({ error: "Finances not created." });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!deleteResponse.rows[0]) {
      return res.status(400).json({ error: "Finances not deleted." });
    }
